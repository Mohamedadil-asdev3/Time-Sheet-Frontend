...
//                     <Typography fontSize={25} fontWeight={600}>SubTask Management</Typography>
//                     <Box sx={{ display: "flex", alignItems: "center", gap: 2 }}>
//                         <Button startIcon={<AddIcon />} variant="contained" size="small" color="primary" >Add SubTask</Button>
//                         <TextField
//                             size="small"
//                             placeholder="Search..."
//                             value={search}
...
//                                                 <Typography color="text.secondary">No SubTask found</Typography>
//                                             </TableCell>
//                                         </TableRow>
//                                     ) : (
//                                         subTasks?.map((data) => (
...
const SubTaskTab = ({ subTasks, setSubTasks }) => {
    ...
    const handleInputChange = (e) => {
        ...
        setCurrentSubTask((prev) => ({ ...prev, [name]: value }));
    };
    ...
    const handleToggleActive = async (id, currentStatus) => {
        ...
        try {
            await updateSubTaskAPI(id, { is_active: newStatus });
            ...
            toast.success(`SubTask ${newStatus ? "activated" : "deactivated"} successfully`);
        } catch (err) {
    ...
    const handleSubmit = async () => {
        if (!currentSubTask.name.trim() || !currentSubTask.displayName.trim()) {
        ...
        try {
            ...
            if (isEditMode) {
                const res = await updateSubTaskAPI(currentSubTask.id, currentSubTask);
                updatedSubTasks = subTasks.map((item) =>
                    item.id === currentSubTask.id ? res.data || res : item
                );
                toast.success("SubTask updated successfully");
            } else {
                const res = await createSubTaskAPI(currentSubTask);
                updatedSubTasks = [...subTasks, res.data || res];
                toast.success("SubTask created successfully");
            }

            setSubTasks(updatedSubTasks);        // Update parent state
            setOpenModal(false);
        } catch (err) {
            console.error("API error:", err);
            toast.error(err.response?.data?.detail || "Failed to save subtask");
        } finally {
            setLoading(false);
        }
    };
                                            ...
                                            >
                                                ...
                                                <TableCell>
                                                    <IconButton
                                                        color="warning"
                                                        size="small"
                                                        onClick={() => handleEditSubTask(data)}
                                                    >
                                                        <EditIcon />
                                                    </IconButton>
                                                </TableCell>
                    ...
                    <Grid container spacing={2} sx={{ mt: 1 }}>
                        ...
                        <Grid size={12}>
                            <TextField
                                ...
                                label="Display Name"
                                name="displayName"
                                value={currentSubTask.displayName}
                                onChange={handleInputChange}
                                required
                            />
                        </Grid>
                        ...
                        <Grid size={12}>
                            <FormControlLabel
                                control={
                                    <Switch
                                        checked={currentSubTask.is_active}
                                        onChange={(e) =>

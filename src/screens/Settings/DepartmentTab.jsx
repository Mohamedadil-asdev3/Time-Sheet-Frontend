...
//                     <Typography fontSize={25} fontWeight={600}>Department Management</Typography>
//                     <Box sx={{ display: "flex", alignItems: "center", gap: 2 }}>
//                         <Button startIcon={<AddIcon />} variant="contained" size="small" color="primary" >Add Department</Button>
//                         <TextField
//                             size="small"
//                             placeholder="Search..."
//                             value={search}
...
const DepartmentTab = ({ department, setDepartments }) => {
    ...
    const handleToggleActive = async (id, currentStatus) => {
        ...
        try {
            await updateDepartmentAPI(id, { is_active: newStatus });
    ...
    const handleSubmit = async () => {
        if (!currentDepartment.name.trim() || !currentDepartment.displayName.trim()) {
        ...
        try {
            ...
            if (isEditMode) {
                // Update
                const res = await updateDepartmentAPI(currentDepartment.id, currentDepartment);
                updatedDepartments = department.map((item) =>
                    item.id === currentDepartment.id ? res.data || res : item
                );
                toast.success("Department updated successfully");
            } else {
                // Create
                const res = await createDepartmentAPI(currentDepartment);
                updatedDepartments = [...department, res.data || res];
                toast.success("Department created successfully");
            }

            setDepartments(updatedDepartments);   // Update parent state
            setOpenModal(false);
        } catch (err) {
                    ...
                    <Grid container spacing={2} sx={{ mt: 1 }}>
                        ...
                        <Grid size={12}>
                            <TextField
                                ...
                                label="Display Name"
                                name="displayName"
                                value={currentDepartment.displayName}
                                onChange={handleInputChange}
                                required
                            />
                        </Grid>

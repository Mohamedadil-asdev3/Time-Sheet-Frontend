...
//                     <Typography fontSize={25} fontWeight={600}>Task Management</Typography>
//                     <Box sx={{ display: "flex", alignItems: "center", gap: 2 }}>
//                         <Button startIcon={<AddIcon />} variant="contained" size="small" color="primary" >Add Task</Button>
//                         <TextField
//                             size="small"
//                             placeholder="Search..."
//                             value={search}
...
import { createTaskAPI, updateTaskAPI } from "../../Api/settingsPageApi";   // ← Import your APIs

const TaskTab = ({ tasks, setTasks }) => {
    ...
    const handleToggleActive = async (id, currentStatus) => {
        ...
        try {
            await updateTaskAPI(id, { is_active: newStatus });
    ...
    const handleSubmit = async () => {
        if (!currentTask.name.trim() || !currentTask.displayName.trim()) {
        ...
        try {
            ...
            if (isEditMode) {
                const res = await updateTaskAPI(currentTask.id, currentTask);
                updatedTasks = tasks.map((item) =>
                    item.id === currentTask.id ? res.data || res : item
                );
                toast.success("Task updated successfully");
            } else {
                const res = await createTaskAPI(currentTask);
                updatedTasks = [...tasks, res.data || res];
